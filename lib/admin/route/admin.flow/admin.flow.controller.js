...
const {propagateInstanceConditions} = require('~/fb-editor-node/lib/runtime/propagate-show')
...
const mapComponentInstanceFor = (renderInstance, pageData) => async (componentInstance) => {
  const {
    preUpdateContents
  } = getInstanceController(componentInstance)

  if (preUpdateContents) {
    componentInstance = await preUpdateContents(componentInstance, pageData, renderInstance)
  }
...
async function getFlowStep (_id, pageData, options = {}) {
...

  {
    const {
      setContents
    } = getInstanceController(flowStep)

    if (setContents) renderInstance = await setContents(renderInstance, pageData)
  }
    ...
    await changeFlowSteps.forEach(async (draftStep, index, array) => {
      if (draftStep.show && typeof draftStep.show === 'object') {
        ...
        if (stepsInstance.$stepSkipFirst || stepsInstance.$stepOptionFirst) {
          const show = deepClone(draftStepSourceInstance.show)
