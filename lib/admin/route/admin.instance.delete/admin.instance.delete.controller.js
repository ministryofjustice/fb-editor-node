...
async function resolveMOJUploadComponent ({_id} = {}, {maxFiles = 1} = {}, referrer = '/') {
  ...
  if (maxFiles > 1) await Promise.all(entryPointInstances.map(deleteMOJUploadConfirmation(_id, referrer))) // 2
  await Promise.all(entryPointInstances.map(deleteMOJUploadSummary(_id, referrer))) // then 1
}
