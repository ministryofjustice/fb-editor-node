...
function resolvePostRedirectToPreviousUrl ({postRedirect, ...parameters}) {
  if (postRedirect) {
    return postRedirect.redirect(getPostRedirectToPreviousUrl(parameters))
  }
}
...
async function postValidation (pageInstance, pageData, postRedirect) {
  ...
  try {
    ...
    const parameters = {postRedirect, _id, _type, pageInstance, pageData, property}

    return transformValueForComparison(value) === transformValueForComparison(previousValue)
      ? resolvePostRedirectToPreviousUrl(parameters)
      : resolvePostRedirectUrl(parameters)
  } catch (e) {
