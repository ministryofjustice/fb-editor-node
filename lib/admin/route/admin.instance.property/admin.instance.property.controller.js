...
async function postValidation (pageInstance, pageData, postRedirect) {
  ...
  let value // `undefined` is acceptable
  if (pageData.getUserDataProperty('delete-property') !== 'yes') {
    const instancePropertyValue = pageData.getUserDataProperty('value')
    ...
    if (instancePropertyKey === 'steps') {
      ...
      value = addUploadSteps(currentValue, transformValueToDataType(instancePropertyValue, schemaProperties, property))
    } else {
      value = transformValueToDataType(instancePropertyValue, schemaProperties, property)
    }
  }

  try {
    await setInstanceProperty(_id, instancePropertyKey, value) // an undefined value means "delete me"
    ...
    if (postRedirect) {
       ...
       *  Where are we going?
       */
      return transformValueForComparison(value) === transformValueForComparison(previousValue)
