...
function createUploadCheckPage (step, url) {
  ...
  return async function createUploadCheckPageForStep (pageInstance) {
    const {
      _id: pageInstanceId,
      steps = []
    } = pageInstance
    ...
    if (steps.includes(step)) {
      ...
      if (id) {
        if (!isUploadCheckPage(getInstance(id))) {
          /*
           *  There's a page at this position and it's not an upload check,
           *  so insert one
           */
          const input = {
            ...bodyInput,
...
function createUploadSummaryPage (step, url) {
  const urlMatch = getUrlMatch(url)
  ...
  return async function createUploadSummaryPageForStep (pageInstance) {
    const {
      _id: pageInstanceId,
      steps = []
    } = pageInstance
...
async function resolveUploadComponent ({_id, url} = {url: '/upload'}, {maxFiles = 1}) {
  const entryPointInstances = getEntryPointInstances()
    .filter(({_type}) => _type !== 'page.error')

  await Promise.all(entryPointInstances.map(createUploadSummaryPage(_id, url)))
  if (maxFiles > 1) await Promise.all(entryPointInstances.map(createUploadCheckPage(_id, url)))
}
...
module.exports = {
  createUploadSummaryPage,
  createUploadCheckPage,
  setData,
  validate,
  postValidation
}
