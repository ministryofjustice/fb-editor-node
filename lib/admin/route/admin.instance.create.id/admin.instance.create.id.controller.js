...
function postValidation (pageInstance, pageData, postRedirect) {
  ...
  function resolveInstanceProperty () {
    if (addProperty === 'steps') {
      return updateInstanceProperty()
    }
  }

  function resolvePostRedirectUrl () {
    if (postRedirect) {
      return postRedirect.redirect(getPostRedirectUrl())
    }
  }

  return resolveCreateInstance(instanceProperties, addOptions)
    .then(resolveInstanceProperty)
    .then(loadServiceData)
    .then(resolvePostRedirectUrl)
}
