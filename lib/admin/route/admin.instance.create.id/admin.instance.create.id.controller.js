    ...
    if (steps.includes(step)) {
      ...
      if (id) {
        if (!isMOJUploadSummaryPage(getInstance(id))) {
          ...
          const input = {
            ...bodyInput,
            _type: 'page.mojUploadSummary', // 'page.summary',
...
async function resolveMOJUploadComponent ({_id, url} = {url: '/mojUpload'}) {
  await Promise.all(
    getEntryPointInstances()
      .filter(({_type}) => _type !== 'page.error')
      .map(createMOJUploadSummaryPage(_id, url))
  )
}
...
function resolveInstanceProperty ({addProperty, ...parameters}) {
  if (addProperty === 'steps') {
    return updateInstanceProperty({...parameters, addProperty})
  }
}
...
function setData (pageInstance, pageData) {
  // console.log('lib/admin/route/admin.create.id/admin.create.id.setData()')
...
function validate (pageInstance, pageData, isPost) {
  // console.log('lib/admin/route/admin.create.id/admin.create.id.validate()')
...
async function postValidation (pageInstance, pageData, postRedirect) {
  // console.log('lib/admin/route/admin.create.id/admin.create.id.postValidation()')
  ...
  await resolveInstanceProperty({addId, addProperty, _id, operation, pageData})
  await loadServiceData()

  if (isMOJUploadComponent(getInstance(_id))) {
    await resolveMOJUploadComponent(getDiscreteInstance(_id))
  }

  return resolvePostRedirectUrl({postRedirect, addProperty, addId, redirectView, _id, _type, pageData})
}
