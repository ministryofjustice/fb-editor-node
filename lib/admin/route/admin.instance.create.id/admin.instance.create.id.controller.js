...
instanceCreateIdController.setData = (pageInstance, pageData) => {
  ...
  if (idSeed === 'name' || idSeed === 'value') {
    ...
    while (propertySeedCheck) {
      if (getInstanceIdByPropertyValue(idSeed, propertySeed)) {
        propertySeed = propertySeed.replace(/_(\d+)$/, (m, m1) => {
  ...
  if (isPage) {
    ...
    if (addProperty === 'steps') {
      ...
      if (existingIds.length) {
        ...
        const existingRadios = jp.query(pageInstance, '$..[?(@._id == "admin.instance.create.id--existing--radios")]')[0]
  ...
  // // Use guids? rather than ever expanding (and potentially misleading) hiearchical value
  // if not toplevel, that is
  // if (!isPage) {
  //   idPrefix = uuid.v4()
  // }
...
instanceCreateIdController.validate = (pageInstance, pageData, POST) => {
  ...
  if (idSeed && POST) {
    if (!idSeedValue) {
      ...
      const schema = getServiceSchema(_type)
      const schemaProperty = schema.properties[idSeed]
      const validationError = validator.validate(idSeedValue, schemaProperty).errors[0]
      if (validationError) {
