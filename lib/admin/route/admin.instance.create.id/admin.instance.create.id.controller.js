...
async function resolveMOJUploadComponent ({_id, url} = {url: '/mojUpload'}) {
  ...
  await Promise.all(entryPointInstances.map(createMOJUploadSummaryPage(_id, url))) // 1
  await Promise.all(entryPointInstances.map(createMOJUploadConfirmationPage(_id, url))) // then 2
}
...
async function postValidation (pageInstance, pageData, postRedirect) {
   ...
   */
  if (isMOJUploadComponent(getInstance(_id))) await resolveMOJUploadComponent(getDiscreteInstance(_id))
