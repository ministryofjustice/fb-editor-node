...
function createUploadCheckPage ({_id: step, url = '/upload'} = {}) {
...
function createUploadSummaryPage ({_id: step, url = '/upload'} = {}) {
  const urlMatch = getUrlMatch(url)
...
async function resolveUploadComponent (pageInstance = {}, {maxFiles = 1} = {}) {
  ...
  if (entryPointInstances.some(isEntryPointStep(_id))) {
    await Promise.all(entryPointInstances.map(createUploadCheckPage(pageInstance)))
    if (maxFiles > 1) await Promise.all(entryPointInstances.map(createUploadSummaryPage(pageInstance)))
  } else {
