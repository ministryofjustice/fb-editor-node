...
function getPostRedirectUrl ({postRedirect, pageData, _id}) {
  const {
    pagesMethods: {
      getUrl = () => '/admin'
    } = {}
  ...
  return discreteId === _id
    ? '/admin'
    : getUrl('admin.instance', {_id: discreteId})
}
...
function handleDeleteInstanceError (e, {postRedirect, ...parameters}) {
  if (postRedirect) {
    const errors = []
...
function setData (pageInstance, pageData, POST) {
  // console.log('lib/admin/route/admin.instance.controller.setData()')
...
function validate (pageInstance, pageData, POST, ajv) {
  // console.log('lib/admin/route/admin.instance.controller.validate()')
...
async function postValidation (pageInstance, pageData, postRedirect) {
  // console.log('lib/admin/route/admin.instance.controller.postValidation()')
  ...
  if (pageData.getUserDataProperty('delete-instance')) {
    try {
      ...
      return resolvePostRedirectUrl({postRedirect, pageData, _id})
    } catch (e) {
      return handleDeleteInstanceError(e, {postRedirect, pageInstance, pageData})
    }
  }
   ...
   *  Compare as strings
   */
  if (instance === sourceInstance) return postRedirect.success()
