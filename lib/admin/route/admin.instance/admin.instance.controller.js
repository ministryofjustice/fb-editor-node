...
} = require('~/fb-editor-node/admin/route/admin.instance.create.id/admin.instance.create.id.controller')

const {
  deleteMOJUploadConfirmationPage
} = require('~/fb-editor-node/admin/route/admin.instance.delete/admin.instance.delete.controller')
...
async function resolveMOJUploadComponent ({_id, url} = {url: '/mojUpload'}, {maxFiles = 1}, referrer = '/') {
  const entryPointInstances = getEntryPointInstances()
    .filter(({_type}) => _type !== 'page.error')

  if (maxFiles > 1) {
    await Promise.all(entryPointInstances.map(createMOJUploadConfirmationPage(_id, url)))
  } else {
    await Promise.all(entryPointInstances.map(deleteMOJUploadConfirmationPage(_id, referrer)))
  }
}
...
async function setData (pageInstance, pageData, POST, REFERRER) {
   ...
   */
  if (isMOJUploadComponent(runtimeInstance)) await resolveMOJUploadComponent(getDiscreteInstance(_id), runtimeInstance, REFERRER)
  ...
  const allInstanceProps = Object.keys(schema.properties)
    ...
    .filter(property => {
      ...
      if (instanceType === 'fileupload' || instanceType === 'mojUpload') {
        excluded = excluded.concat(['repeatable'])
      }
      if (excluded.length) {
