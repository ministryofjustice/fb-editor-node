...
async function resolveUploadComponent ({_id, url} = {url: '/upload'}, {maxFiles = 1}, referrer = '/') {
  const entryPointInstances = getEntryPointInstances()
    .filter(({_type}) => _type !== 'page.error')

  if (maxFiles > 1) {
    await Promise.all(entryPointInstances.map(createUploadCheckPage(_id, url)))
  } else {
    await Promise.all(entryPointInstances.map(deleteUploadCheckPage(_id, referrer)))
  }
}
