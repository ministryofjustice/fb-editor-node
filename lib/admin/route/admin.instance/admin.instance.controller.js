...
async function resolveUploadComponent (pageInstance = {}, {maxFiles = 1}, referrer = '/') {
  ...
  if (entryPointInstances.some(isEntryPointStep(_id))) {
    if (maxFiles > 0) await Promise.all(entryPointInstances.map(createUploadCheckPage(pageInstance)))
    ...
    if (maxFiles < 1) await Promise.all(entryPointInstances.map(deleteUploadCheckPage(pageInstance, referrer)))
  } else {
