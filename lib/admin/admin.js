...
function createPostRedirectForMOJFileUpload (stepId, controller) {
  return function createPostRedirect (res, req, url, next) {
    return {
      redirect: (url) => {
        const [
          ,
          urlMatch = 'moj-file-upload'
        ] = url.match(/\/(.*)\//) || []
        ...
        getEntryPointInstances()
          .filter(({_type: entryPointInstanceType}) => entryPointInstanceType !== 'page.error')
          .forEach((entryPointInstance) => {
            const {
              _id: entryPointInstanceId,
              steps = []
            } = entryPointInstance

            if (steps.includes(stepId)) {
              const index = steps.findIndex((step) => step === stepId) + 1
              ...
              if (id) {
                ...
                if (type !== 'page.summary') {
                  const input = {
                    ...bodyInput,
                    _type: 'page.summary',
                    addId: entryPointInstanceId,
                    addProperty: 'steps',
                    insertAt: index
                  }
              ...
              if (pageData) {
                ...
                controller.setData(entryPointInstance, pageData)
                controller.validate(entryPointInstance, pageData, true)
                controller.postValidation(entryPointInstance, pageData)
              }
            }
          })
...
function pageHandler (req, res, next) {
  ...
  if (!handlerData) {
    ...
    let pageInstance = deepClone(pageInstancesRouteMap[route])
    ...
    if (controller.setData) {
      pageInstance = controller.setData(pageInstance, pageData, POST, REFERRER)
    ...
    if (controller.validate) {
      pageInstance = controller.validate(pageInstance, pageData, POST, ajv)
    }
    ...
    if (POST || pageInstance.executePostValidation) {
      ...
      if (_type === 'mojFileupload') {
        /*
         *  MOJ File Upload: add a 'summary' step
         */
        const _page = pageData.getUserDataProperty('addId')

        const createPostRedirect = createPostRedirectForMOJFileUpload(_page, controller)
 ...
 */
{
  const routeMap = deepClone(pageInstancesRouteMap)
